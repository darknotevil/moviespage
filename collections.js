(function () {
    ...
    /**
     * Фокусируемая карточка "Ещё" в конце ряда.
     * Свой обработчик hover:enter, поэтому работает с пульта (не зависит от строки).
     * data.action - что сделать по нажатию.
     */
    function MoreCard(data) {
      ...
      this.create = function () {
        item = Lampa.Template.js('cub_more_card');
        item.find('.cub-more-card__title').text(data.more_title || Lampa.Lang.translate('more'));
        item.addEventListener('hover:focus', function () {
          if (_this.onFocus) _this.onFocus(item, data);
        });
        item.addEventListener('hover:enter', function () {
          if (data.action) data.action();
        });
        item.addEventListener('visible', function () {
          if (_this.onVisible) _this.onVisible(item, data);
        });
      };
    ...
    /**
     * Элемент-результат для ряда, который отрисуется как MoreCard.
     */
    function moreElement(title, action) {
      return {
        more_card: true,
        more_title: title,
        action: action,
        cardClass: function cardClass(elem, params) {
          return new MoreCard(elem, params);
        }
      };
    }
    ...
    function favoriteLines() {
      var book = Lampa.Favorite.get({
        type: 'book'
      }) || [];
      var defs = [{
        title: 'Избранное — Фильмы',
        filter: 'movies'
      }, {
        title: 'Избранное — Сериалы',
        filter: 'tv'
      }];
      var lines = [];
      defs.forEach(function (d) {
        var results = Lampa.Utils.filterCardsByType(book, d.filter).slice();
        if (!results.length) return;
        results.push(moreElement(Lampa.Lang.translate('more'), function () {
          Lampa.Activity.push({
            url: '',
            title: d.title,
            component: 'favorite',
    ...
    function main(params, oncomplite, onerror) {
      var user = Lampa.Storage.get('account', '{}');
      if (!user.token) return onerror();

      // 1. список коллекций пользователя
      network.silent(api_url + 'list?cid=' + user.id, function (list) {
        var cols = list.results || [];
        if (!cols.length) return onerror();
        ...
        // 2. содержимое каждой коллекции
        var status = new Lampa.Status(cols.length);

        status.onComplite = function () {
          var lines = [];
          cols.forEach(function (col) {
            var view = status.data['c' + col.id];
            if (!view || !view.results || !view.results.length) return;
            var title = Lampa.Utils.capitalizeFirstLetter(col.title);
            var results = view.results.slice();

            // Если в коллекции больше карточек, чем пришло на первой странице,
            // добавляем фокусируемую карточку "Ещё" -> полный грид коллекции.
            if (col.items_count > view.results.length) {
              results.push(moreElement(Lampa.Lang.translate('more'), function () {
                Lampa.Activity.push({
                  url: col.id,
                  collection: col,
            ...
            lines.push({
              ...
              line_type: 'cards',
              nomore: true,
              pinned: col.id == PINNED
            });
          });
          if (lines.length) oncomplite(lines);else onerror();
        };

        cols.forEach(function (col) {
          network.silent(api_url + 'view/' + col.id + '?page=1', function (view) {
            status.append('c' + col.id, view);
    ...
    function component$2(object) {
      ...
      comp.create = function () {
        ...
        Api.main(object, function (lines) {
          var fav = favoriteLines();
          var out;

          // Порядок: закреплённая коллекция (3061) -> избранное -> остальные коллекции
          if (lines.length && lines[0].pinned) out = [lines[0]].concat(fav, lines.slice(1));else out = fav.concat(lines);
          _this.build(out);
        }, function () {
          var fav = favoriteLines();
          if (fav.length) _this.build(fav);else _this.empty();
        });
        return this.render();
      };
    ...
    function startPlugin() {
      ...
      // Запуск приложения сразу в «Коллекции»: переиспользуем штатную настройку
      // «Стартовая страница» = «Мои торренты» (она не используется). Выставляем
      // deep-link на eval плагина — это раньше Activity.last() на загрузке.
      if (!window.start_deep_link && Lampa.Storage.get('start_page') === 'mytorrents') {
